...
## Этап 9 — Персистентность фронтира: чекпоинт и возобновление (POST_MVP)

**Цель:** длинный обход можно прервать (Ctrl-C, `SIGTERM`, креш) без потери
всего прогресса. `--checkpoint PATH` включает периодическую (раз в
`--checkpoint-every`, дефолт 100 обработанных URL) и финальную запись снимка
обхода на диск; `--resume` (вместе с `--checkpoint PATH`) поднимает снимок и
продолжает обход с сохранённого фронтира, не переобходя посещённое.

Полная спецификация (что именно персистится, формат снимка, инварианты
конкурентности, границы, разбор тестов) — в
`docs/plans/этап-9-персистентность-фронтира.md`, план не устарел после
реализации и оставлен как рабочая спецификация. Кратко:

- Новый модуль `checkpoint.py` — сериализация в JSON + атомарная запись
  (tmp-файл в целевой директории + `os.replace`), доменная ошибка
  `CheckpointFormatError` на битом/неполном/несовместимом по версии файле.
- Снимок включает: непройденный фронтир (очередь + in-flight-задачи, взятые
  воркером, но ещё не завершённые — иначе они теряются при креше между
  `get()` и концом `_process`), множество виденных URL, счётчики `CrawlStats`,
  накопленные `edges`/`pages`, состояние sitemap-обнаружения, seeds/max_depth
  для контроля совместимости при resume.
- Состояние `PerDomainLimiter` (`_next_dispatch`/`_backoff`) сознательно **не**
  персистится — эфемерная политесс-метрика одного прогона, `time.monotonic()`
  не сопоставим между процессами; resume стартует с чистым лимитером.
- Несовпадение `seeds`/`max_depth` между чекпоинтом и resume-запуском,
  отсутствующий или битый файл на `--resume` — fail fast, ненулевой код
  возврата, чекпоинт-файл не портится.
- Без `--checkpoint`/`--resume` поведение и производительность неотличимы от
  Этапа 8 — никакого лишнего I/O.

**Готово когда:** обход с `--checkpoint state.json`, прерванный на середине,
оставляет валидный атомарно записанный чекпоинт; `--checkpoint state.json
--resume` продолжает его с сохранённого фронтира и даёт те же финальные
`stats`/`edges`/`pages`, что непрерывный обход того же графа; несовпадение
seeds/max_depth или битый/отсутствующий чекпоинт на resume — понятная ошибка
без порчи файла.
